...
function Factory({ token, setSuccess }) {
  ...
  const submit = async () => {
    ...
    if (!name || !description) {
      ...
      try {
        ...
        const { status, ipfsHashMetadata, signature } = await (
        ...
        console.log(e)
        alert(
          'There was an error while uploading your assets. Please refresh and try again'
        )
      }
    } else {
  ...
  return (
    <Container>
      ...
      <div>
        <Button disabled={loading} onClick={submit} purple>
          {' '}
