...
function Authentication({ isShowing, toggle, className }) {
  ...
  const { authenticate, simple_authenticate, dispatch } =
  ...
  async function authenticateAfterConnect(ethereumAccountId, isSimpleAuth) {
    ...
    if (isSimpleAuth) {
      var { success, token, message } = await simple_authenticate(
        ethereumAccountId,
        'coinbase'
      )
    } else {
  ...
  async function connectWalletLink() {
    try {
      ...
      const ethereum = walletLink.makeWeb3Provider(
        'https://mainnet.infura.io/v3/85785677004c47b2b33a41a174e1a775',
        1
      )
